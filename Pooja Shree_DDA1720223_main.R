...
# that would help global mart manage the revenue and inventory accordingly.

## AIM:
# Aim of this case study is to build a efficient model to forecast the sales and the demand for the next 6 months,
# which would help global mart to manage the future revenue.

#************ Data understanding ************# ----
# Global Super Store is a data set which has around 51,000 values. 
...
# 1.Checking for missing column names
  names(superstore)
...
# 3.Looking for Missing values in the dataframe
...
# Extract the Year from the Order.Date and create a column year for the aggregation of data
  ...
  # colnames(superstore_agg_year) = c("Market_Segment","Year","Sales","Profit","Quantity")
  
  # superstore_agg_year$CV_Profit <- sd(superstore_agg_year$Profit)*100/abs(mean(superstore_agg_year$Profit)) # Profit Co-efficient of Variation
...
# Converting the Smoothed series into a data frame and adding the Year_Month coloum from the original sales data.
...
# Plotting the ACF and PACF plots for caluclation of the p,q,d values 
